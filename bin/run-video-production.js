...
if (options.help || !options.root) {
  console.log(`用法：
  npm run exec:video -- <production-root> [--provider mock|command] [--command ./worker.js] [--all] [--continue-on-error] [--limit 4] [--task TASK_ID]
...
function parseArgs(args) {
  const options = { root: "", provider: "mock", command: "", commandArgs: [], all: false, continueOnError: false, limit: Number.POSITIVE_INFINITY, taskIds: [], maxPasses: 12, help: false };
