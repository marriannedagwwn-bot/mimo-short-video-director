...
`Visual Guardrails` 是固定角色语义唯一生成阶段。视觉模型会结合用户角色描述、参考分析、脚本还原和创意简报，一次性输出 `requiredTraits`、`allowedTraits` 与 `forbiddenTraits`；可以使用模型常识理解“狼娘”等开放概念，但不依赖本地写死的物种关键词字典，且用户明确肯定或否定的设定优先。边界只围绕「固定角色」一栏里的一个角色：`characterName` 必须逐字等于服务端从这一栏取出的名字，提示词会直接给出这个名字；创作限制里的固定搭档、宠物、家人等其它角色不进这份边界，它们的外观由后续阶段按创作限制原文处理。「固定角色」一栏目前只能登记一个角色。
