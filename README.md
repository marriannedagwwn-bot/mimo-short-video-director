...
MODEL_PRICE_CNY_PER_MILLION=qwen3.7-max=2.4/9.6,qwen3.7-plus=0.8/2
```

模型 ID 必须与实际使用的 `*_MODEL` 完全一致。未配置单价的模型只显示 token 数、不显示金额——宁可不报，也不报一个可能过期的错数字。单条格式错误只会被跳过并告警，不影响启动。
