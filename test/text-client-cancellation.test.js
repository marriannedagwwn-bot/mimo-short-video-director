...
const CLIENTS = [
  { provider: "MiMo", Client: MimoClient, model: "mimo-v2.5" },
  { provider: "DeepSeek", Client: DeepSeekClient, model: "deepseek-v4-flash" },
  { provider: "Qwen", Client: QwenClient, model: "qwen3.7-max" }
];
...
function complete(response, spec) {
  response.setHeader("content-type", spec.provider === "Qwen" ? "text/event-stream" : "application/json");
  response.end(spec.provider === "Qwen"
...
for (const spec of CLIENTS) {
  ...
  test(`${spec.provider}: pause aborts an active response body with its original reason`, { timeout: 5_000 }, async (t) => {
    ...
    const server = await fixture(t, (_request, response) => {
      response.writeHead(200, { "content-type": spec.provider === "Qwen" ? "text/event-stream" : "application/json" });
      response.write(spec.provider === "Qwen" ? sseChunks({ content: "partial" }).split("\n\n")[0] + "\n\n" : '{"choices":[');
    });
    const controller = new AbortController();
    ...
    await server.requested;
    // Qwen's heartbeat proves the client consumed an SSE chunk before cancellation.
    if (spec.provider === "Qwen") await readStarted.promise;
...
test("Qwen: usage received before a user abort is accounted once and never becomes a completion", { timeout: 5_000 }, async (t) => {
  const controller = new AbortController();
  const reason = controlReason();
  const server = await fixture(t, (_request, response) => {
  ...
  const spec = CLIENTS.find((item) => item.provider === "Qwen");
  await assert.rejects(start(clientFor(spec, server.baseUrl), {
    signal: controller.signal,
...
for (const interrupted of ["disconnect", "incomplete EOF"]) {
  test(`Qwen: provider ${interrupted} retains received usage and its existing error classification`, { timeout: 5_000 }, async (t) => {
...
test("Qwen: a timeout during SSE consumption retains the existing retryable stream error", { timeout: 5_000 }, async (t) => {
  const server = await fixture(t, (_request, response) => {
    response.writeHead(200, { "content-type": "text/event-stream" });
...
test("Qwen: coordinator preserves a cancelled stream reason and does not automatically retry", { timeout: 5_000 }, async (t) => {
  const controller = new AbortController();
  const reason = controlReason();
  const server = await fixture(t, (_request, response) => {
    response.writeHead(200, { "content-type": "text/event-stream" });
  ...
  await assert.rejects(start(client, {
    ...
    client, request: { prompt: "fixture", model: spec.model }, provider: "Qwen", stage: "fixture", validate: (value) => value
  })), (error) => error === reason);
  assert.equal(server.calls(), 1);
});
