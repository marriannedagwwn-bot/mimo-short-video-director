...
### 同日处理：提示词补上 beat 来源约束（代码由 Codex 编写）

根因：`storyboardValidationErrors` 一直要求每个 beat 的 `sourceSceneIds` 包含在所在片段的 `sourceSceneIds` 里，而设计分镜提示词第 9 条只写「可以一对多和多对一」，从没说过这条——校验器要求了一件提示词没说的事。被拦后的拒绝理由「本片段只允许 S4」又只暗示「把下一场从 beat 删掉」这一种改法。

改动：

- `src/storyboard-prompts.js` 第 9 条末尾补一句：每个 beat 的 sourceSceneIds 都必须出现在其所在片段的 sourceSceneIds 中；beat 演到下一场（如片段末尾的过渡拍已开始演 S2 的动作），就把该场也补进本片段的 sourceSceneIds，不要只标在 beat 上。所有模型共用。
- `src/storyboard-contract.js` 的 `STORYBOARD_BEAT_SOURCE_OUT_OF_SHOT` 拒绝理由改为：beat 引用了不在本片段中的场次 X；本片段现有场次 Y；若 beat 确实演到了该场，就将其补进本片段的 sourceSceneIds；若没演到，就从 beat 的 sourceSceneIds 中去掉。判定逻辑、诊断码、路径不变；正解不唯一，仍不自动修。
- 修订阶段冻结来源场次（`editable()` 排除全部 `sourceSceneIds` 路径），审阅只出报告，所以只有设计分镜需要这条约束。
- 新增 3 条测试；本机 `node --test test/*.test.js`：1787 tests / 1781 pass / 0 fail / 6 skipped。两份撞错的真实输出用新校验器重放，判定不变（同码同路径），理由换成新的。

真实回放（同一份冻结输入，只调用设计分镜，MiMo 开思考 ×3 并发）：

| 次 | 结果 | 调用 | 耗时 | completion token | 片段 / 拍 / 总长 |
|---|---|---|---|---|---|
| 1 | 通过 | 1 | 418 秒 | 15645 | 8 / 24 / 96 秒 |
| 2 | 通过 | 1 | 389 秒 | 15154 | 7 / 23 / 95 秒 |
| 3 | 通过 | 1 | 393 秒 | 14619 | 9 / 20 / 98 秒 |

- 事先登记的通过线（3 次的第一次调用里该错 0 次）达到：0/3。改动前结构合格的 4 份里 2 份撞上。
- **要继续观察**：这 3 份里跨场过渡拍（beat 标两个场次）与多场次片段都是 0，每段只承接一个场次；改动前 5 份里分别有 2 份、1 份（其中一份把相邻场次合并进同一片段，这是 4.0 允许的）。模型是靠不再跨场来避开这个错的。样本太小，分不清是不是这句提示词把相邻场次合并也压掉了；如果后续分镜明显变碎、该合并的相邻剧情总被切开，再回头看这句的措辞。
- 耗时与 token 比改动前那批低（约 400 秒、1.5 万 vs 约 600 秒、2 万），样本小，不作结论。
