 ...
 * 每拍的「最容易被写成的那个错误版本」。
 *
 * 依据：十篇外部高分改写稿 61 拍里 **26 拍（43%）**的每拍功能说明带否定条款，形式统一为
 *「不是 X，而是 Y」，每条都在挡离这一拍最近的那个陈词滥调（「笑点不另起一条支线」
 *「结尾回收前面的游戏，而不是另加一个普通告别」）。现有候选的 dramaticFunction 全是
 *「建立场景，引入情境」这类通用剧作词汇，贴到任何候选上都成立，**没有一条说这一拍不许写成什么**——
 * 实测后果是某候选最后一拍的红薯从口袋里凭空出现，没有任何东西禁止「再加一个温情道具」。
 *
 * **它必须是独立字段，不能并进 dramaticFunction。** 后者是 storyCandidateStructureSignature 的输入，
 * 结构签名把全部拍的 dramaticFunction 逐字拼起来做两两比对；塞进否定条款会让每条串更长更独特、
 * 碰撞概率骤降，等于悄悄削弱一个现有闸门。
 *
 * **也不能设成必填。** ensureStoryCandidateContract 跑 strict schema，而它被 validateBoundCandidate
 * 调用——那是用户选中候选、生成 Full Story 时跑的。必填会让所有已签发的旧候选在展开剧情时当场失败。
 *
 * 三条判定全是计数与字符串比较，零语义：**判不出这条否定条款写得对不对**，只能挡住
 * 写得太少、跨拍复读、以及把 dramaticFunction 换个说法否定一遍这三种退化。
 */
// 阈值 2，依据是外部高分稿的 43%（26/61 拍），候选是 5–7 拍，取下界。
//
// 中途曾降到 1：qwen3.7-max 上三个包 2/3 硬失败（一个包只写 1 拍，另一个给不想写的拍输出空串）。
// 但那是**模型指令跟随能力**，不是阈值定错——同一批包换 qwen3.8-max 之后，20 处该省略的键
// **全部干净省略、0 处写空串**，而 3.7 在同一个包上写了一堆空串。为了迁就较弱的模型而降标准，
// 正是 §5 第 1 条禁的那件事，所以改回 2。
export function requiredClicheClauseCount(beatCount) {
  return beatCount >= 5 ? 2 : 1;
}

export function validateVariantClicheClauses(variants) {
  const details = [];
  ...
  (Array.isArray(variants) ? variants : []).forEach((variant, index) => {
    ...
    const seen = new Map();
    let filled = 0;
    outline.forEach((beat, order) => {
      // 空串等于没写：实测模型会给不想写的拍输出 "" 而不是省略整个键，schema 因此不再硬失败。
      const clause = String(beat?.clicheToAvoid || "").trim();
      if (!clause) return;
      filled += 1;
      const key = normalizeStoryReviewEcho(clause);
      if (key && key === normalizeStoryReviewEcho(beat?.dramaticFunction)) {
        push(
          "STORY_CANDIDATE_CLICHE_ECHOES_FUNCTION",
          `/variants/${index}/storyOutline/${order}/clicheToAvoid`,
          `${label} 第 ${order + 1} 拍的 clicheToAvoid 与同拍 dramaticFunction 去掉标点后相同；`
          + "它要写的是这一拍最容易被写成的那个错误版本，不是把功能说明否定一遍"
        );
        return;
      }
      if (key && seen.has(key)) {
        push(
          ...
          `/variants/${index}/storyOutline/${order}/clicheToAvoid`,
          `${label} 第 ${order + 1} 拍与第 ${seen.get(key) + 1} 拍的 clicheToAvoid 是同一句；`
          + "每一条都要针对那一拍自己的滑坡风险"
        );
        return;
      }
    ...
    const required = requiredClicheClauseCount(outline.length);
    if (filled >= required) return;
    push(
      "STORY_CANDIDATE_CLICHE_COVERAGE",
      `/variants/${index}/storyOutline`,
      `${label} 只有 ${filled} 拍写了 clicheToAvoid，至少需要 ${required} 拍；`
      + "不要求每拍都写，但通篇不写等于没有任何东西挡住最顺手的那个陈词滥调"
    );
  });
  if (!details.length) return;
  throw new OutputContractError(
    `themeVariants 的 clicheToAvoid 不合契约：${details.map((d) => `${d.path} ${d.reason}`).join("；")}`,
    details
  );
}
