...
function recommendedCommands(run = {}, state = {}) {
  ...
  if (state.failed?.length) {
    return [`npm run exec:video -- ${root} --provider command --command node --command-arg ./workers/command-worker-template.mjs --all --retry-failed`];
  }
  if (state.ready?.length) {
    return [`npm run exec:video -- ${root} --provider command --command node --command-arg ./workers/command-worker-template.mjs --all`];
  }
