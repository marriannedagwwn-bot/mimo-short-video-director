...
1. **召回率很低。** 改完后两次回放共 8 个候选槽位只命中 1 条，且那个候选已知有 3 处矛盾，只抓到 1 处（另两处两次都没抓到）。
   命中的那次落在正确的候选上、锚到了具体拍号，**所以机制是通的，不是摆设**；但它**不是完备闸门**，不得据此宣称「自洽问题会被拦住」。
2. **`pass` 闸门在 live 从未被触发过**——两次回放模型都直接判了 `drop`，闸门只有单元测试证明有效。
3. **样本是 n=3 回放、单一参考片包**，不足以支撑任何比例结论。**不得为了提高召回率去加词表或反复改提示词**（§5 第 2 条与「看到错误日志就直接改提示词」）。

**同一次回放还暴露了旧契约的一个更根本的问题，但没有确定性修复**：回放 1（旧契约）给四个候选提炼出 **8 条各不相同**的「原片机制」，
