...
- **自主分镜 4.0 的「设计分镜」同样接入（2026-09-24）**：MiMo 开思考在 run-7336db89 上两次都被拦——第一次把 `locations`/`props` 写到顶层而不是 `visualDesign` 里（结构错），带诊断重试改对了结构却又出现 `STORYBOARD_BEAT_SOURCE_OUT_OF_SHOT`，2 次预算用完、整份 Plan 失败。`storyboardDesignModelSchema()`（`src/contracts/storyboard-design-model-schema.js`）从 `storyboardDesignSchema` 派生：`pattern` 改 `minLength: 1`；去掉没在 MiMo 上实测过的 `uniqueItems`/`exclusiveMinimum`（只放宽模型那侧，服务端照旧拦）；派生结果只允许已实测的关键字，出现其他关键字或别的 pattern 直接抛错；`shotPlan` 条数不锁。`src/storyboard-workflow.js` 的 `call()` 新增可选 `responseSchema`，**只有** `storyboardDesign`（及其重试）带，角色事实、审阅、修订、终审、单镜写稿都不带。提示词、严格校验、重试预算不变。离线：失败那次第一份输出被派生 Schema 拒、第二份放行。同输入真实回放 2 次：2/2 设计通过，结构错 0/3 次调用，其中一次先被 `STORYBOARD_BEAT_SOURCE_OUT_OF_SHOT` 拦下、重试通过。**反复出现、未处理**：当天 MiMo 4 份设计里 2 份是同一形状——片段末尾的过渡拍标了「本场 + 下一场」，片段自身的 `sourceSceneIds` 却只写本场；正解不唯一（给片段补场次，或把下一场从 beat 去掉），不得自动修，目前只靠带诊断重试。代码由 Codex 按任务说明编写，审查与回放由 Claude 完成。
