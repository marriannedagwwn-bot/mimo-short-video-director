...
- **输出上限统一默认 32768（2026-09-23）。** Qwen（全局与各阶段）、MiMo（全局、剧情、动画）、DeepSeek 的 max tokens 默认值都改为 32768（此前 Qwen/DeepSeek 16384、MiMo 8192/12288）；评审三档仍是 `Math.max(…, 32768)`，旧版静态帧编译沿用自己的配置。**调大而不取消**：流式请求已不设总时长，上限是模型陷入重复输出（2026-08-30 那类）时唯一的刹车，取消后 MiMo 会一路写到 131072 才停。上限只在输出本来就会超过时才起作用，正常调用不多花钱。MiMo 的上限包含推理 token（开思考跑候选实测约 2.5 万），千问看起来不含（未对照官方文档）。在用的 7 个模型（qwen3.7-max、qwen3.7-plus、mimo-v2.5、mimo-v2.5-pro、mimo-v2.6-pro、deepseek-v4-flash、deepseek-v4-pro）实测 32768 与 65536 都被接受，所以各重试路径把上限再抬 1.35–1.5 倍也不会被拒。
