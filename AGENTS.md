...
**每拍的「最容易被写成的那个错误版本」`clicheToAvoid`（2026-09-10）**：`storyOutline[]` 新增**可选**字段，
与 `failureSignal` 同族——后者写「这条机制没迁移成功会长成什么样」，它写「这一拍写砸了会长成什么样」，
形式是「不是 X，而是 Y」。依据是十篇外部高分改写稿 61 拍里 **26 拍（43%）**带这种否定条款，
而现有候选的 `dramaticFunction` 全是「建立场景，引入情境」这类通用剧作词汇，贴到任何候选上都成立，
**没有一条说这一拍不许写成什么**——实测后果是某候选最后一拍的红薯从口袋里凭空出现。

**两条结构约束决定了它必须长成这样，都不是可选的：**

- **不能并进 `dramaticFunction`。** 那是 `storyCandidateStructureSignature` 的输入，结构签名把全部拍的
  `dramaticFunction` 逐字拼起来做两两比对；塞进否定条款会让每条串更长更独特、碰撞概率骤降，
  **等于悄悄削弱一个现有闸门**（§5 第 1 条）。
- **不能设成必填，闸门也不能放进 `ensureThemeVariantsMatchProfile`。** `ensureStoryCandidateContract` 跑
  strict schema，而它与那个共享校验器都被 `validateBoundCandidate`（`server.js:301`）调用——那是**用户选中
  候选、生成 Full Story 时**跑的。放进去会让**所有已签发的旧候选在展开剧情时当场失败**。
  第一版挂在共享校验器的 `if (upstream)` 分支上，实测**直接打挂 10 条既有测试**：`upstream` 在那些用例里
  表示「要做溯源核对」，不表示「这是一次新生成」，两件事不能共用一个开关。现在闸门只在
  `createVariants` 的 `finalize` 里跑。

三条判定全是计数与字符串比较，**零语义**：每个候选至少 2 拍带非空条款（少于 5 拍时 1 拍，
`STORY_CANDIDATE_CLICHE_COVERAGE`）；同一候选内两两不同（`..._DUPLICATED`）；与同拍 `dramaticFunction`
归一化后不得相同（`..._ECHOES_FUNCTION`）。归一化复用 `normalizeStoryReviewEcho`。
**空串按「没写」计，不硬失败**——实测模型会给不想写的拍输出 `""` 而不是省略整个键，
schema 因此只要求 `string`；硬失败在这里买不到任何东西，只会让整批被拒。

它**要送给候选对照评审**，理由与 `failureSignal` 逐字相同：把「陷阱」给评审看、把「答案」藏起来。

**阈值 2 是模型能力实测的结果，不是拍脑袋。** 中途曾降到 1：`qwen3.7-max` 上三个包 2/3 硬失败
（一个只写 1 条，一个给不想写的拍写空串）。但换 `qwen3.8-max-0902` 后**两个包 8/8 个候选全部写够 2 条、
0 处空串**，而 3.7 在同一个包上写了一堆空串。为迁就较弱的模型降标准正是 §5 第 1 条禁的那件事，因此改回 2。

**四条必须如实记着的代价与局限：**

1. **它没有被证明能提高候选质量。** 当初的病灶是「结尾凭空多出一个温情道具」，而实测里 3.7 的某个候选
   仍然让长辈在倒数第二拍掏出一件新奖励物——**而那一拍恰好写了条款，没挡住**。
2. **模型在按数量交差。** 3.8 严格贴线：提示词说 1 就写 1/1/1/1，说 2 就写 2/2/2/2，八个候选无一例外；
   而外部高分稿是 43% 且分布不均。**闸门数得出数量，判不出「这一拍真有没有滑坡风险」。**
3. **它把候选阶段推向 3.8，而那有实打实的代价。** 3.8 比 3.7 慢 **5–7 倍**（140 秒 → 780–1060 秒），
   输入 token 几乎相同而 completion 翻 2.15 倍——§2.7 实测它 82% 的 completion token 是推理。
   而且 `.env` 缺它的价目，用量记账拿到的是 `costCny: null`（`docs/待解决项.md` 第 8 条）。
4. **`qwen3.8` 不是全面更好。** 同一批实测里它**两次都**漏掉 `narrativeMode` 的分布要求
   （明写至少 2 个 `slice_of_life`，它只给 1 个），而 3.7 在同一个包上满足了。选模型时不能只看一条指令。

该字段**不进入 Full Story 提示词的任何具名投影**，只随 `JSON.stringify(variant)` 整体带入，
与 `estimatedSeconds` 同一形状；**它能不能影响展开阶段未经验证**。
